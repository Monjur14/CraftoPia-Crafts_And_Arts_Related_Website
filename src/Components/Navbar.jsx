  ...
  return (
    <div className="contain flex justify-between items-center py-2 lg:py-3 px-3 lg:px-0">
      ...
      <h1 className="text-[2.2rem] block lg:hidden cursor-pointer ml-2" onClick={toggleNavbar}><GiHamburgerMenu /></h1>
        ...
        {user && <NavLink to={"/additem"} className={({isActive}) => isActive ? "font-bold cursor-pointer text-cyan-700" : "cursor-pointer"} onClick={toggleNavbar}>Add Item</NavLink>}
        {user && <NavLink to={"/mylist"} className={({isActive}) => isActive ? "font-bold cursor-pointer text-cyan-700" : "cursor-pointer"} onClick={toggleNavbar}>My List</NavLink>}
            </ul>
            {
        user?.email ? <div className="flex gap-2 w-full justify-center mt-5">
          ...
          <Link to={"/register"} className="bg-cyan-700 text-white font-semibold text-xl w-40 text-center py-2 rounded-md" onClick={toggleNavbar}>Register</Link>
        </div>
      }
        </div>
    </div>
